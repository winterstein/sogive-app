...
public class BasketPublishedActor extends Actor<Basket> {
	...
	private void notifyUs(Ticket ticket) {
		try {
			...
			Emailer emailer = Dep.get(Emailer.class);
			SimpleMessage email = new SimpleMessage(emailer.getBotEmail().toString(), "support@sogive.org",
					"Shop " + AppUtils.getServerType() + " " + AppUtils.getFullHostname() + " - card bought :)",
