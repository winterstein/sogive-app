...
//		Add idempotency key to request (https://stripe.com/docs/api#idempotent_requests)
		...
		Log.d(LOGTAG, "Create... PaymentIntent idempotencyKey:" + idempotencyKey + " customer: " + pm.getCustomer()
		...
		if (user != null && pi.getCustomer() != null) {
			...
			Map cInfo = new ArrayMap("customerId", pi.getCustomer(), "email",
					cobj == null ? pi.getReceiptEmail() : cobj.getEmail());
	 ...
	 * @param sa
	 * @param user
	 * @param idempotencyKey E.g. the Donation or Basket id. Repeated calls with the
	 *                       same i-key _within 24 hours_ should be harmless. See
	 *                       https://stripe.com/docs/api#idempotent_requests
	 * @return
	 * @throws Exception
	 */
	public static Charge collectLegacy(Money amount, String description, StripeAuth sa, Person user,
			String idempotencyKey) throws Exception {
...
//		add header Idempotency-Key:
		...
		if (user != null && c.getCustomer() != null) {
			...
			Map cInfo = new ArrayMap("customerId", c.getCustomer(), "email",
					cobj == null ? c.getReceiptEmail() : cobj.getEmail());
			user.put("stripe", cInfo);
		}
		// TODO turn into a map
		return c;
	...
	/**
	 * A StripeAuth created after the 2020-12 update may contain an ID for a
	 * PaymentIntent or a PaymentMethod. To make a new payment we always need the
	 * PaymentMethod - this abstracts away the extra retrieval steps.
	 * 
	 * @param sa
	 * @return
