...
public class DonationServlet extends CrudServlet<Donation> {
	...
	/**
	 * Collect money! One off or repeat donations are OK
	 * 
	 * @param state       Can be null
	 * @param donation
	 * @param user        Cannot be null (use email if not logged in)
	 * @param email       Can be null for e.g. Good-Loop "donations" But must be set
	 *                    for "proper" SoGive donations
	 * @param takePayment False = Payment has already been made client-side (initial
	 *                    donation). True = Payment should be taken now (repeat
	 *                    donation)
	 * @return unposted message, to allow it to be posted after the donation is
	 *         published
	 * 
	 */
	public static List<MsgToActor> doPublish3_ShowMeTheMoney(WebRequest state, Donation donation, XId user,
			String email, boolean takePayment)
...
//		if (donation.getDate()==null) { // date = published date not draft creation date
		...
		if (donation.getDonor() == null) {
			Map info = new ArrayMap("name", donation.getDonorName());
		...
		// Do we have a recipient on the donation? Try to recover if it looks like we
		// don't.
		...
		if (donation.isPaidElsewhere()) {
			...
			mc.takePayment = takePayment; // Only create a new PaymentIntent if the invoking code says (ie if this is a
											// repeat donation)
	...
	static void doUploadTransfers2_email(Donation donation, String emailAddress) throws AddressException {
		...
		String bodyHtml = "<div><h2>Thank You for Donating!</h2><p>We've received your donation of " + amount + " to "
				+ beneficiary + tip + ".</p><p>Payment ID: "
				+ Utils.or(donation.getPaymentId(), donation.getPaymentMethod(), donation.getId()) + "<br>Donation ID: "
				+ donation.getId() + "</p></div>";
	...
	/**
	 * Remove sensitive details for privacy TODO Export-CSV should e.g. set a prop
	 * on the servlet which says "don't sanitize, I need full data"
	 * 
	 * @param hits2
	 * @param state
	 * @return
	...
	protected JThing<Donation> cleanse(JThing<Donation> thing, WebRequest state) {
		...
		if (q != null && q.endsWith("purpose:admin")) {
			...
			for (AuthToken authToken : tokens) {
				if (!authToken.xid.isService("email"))
					continue;
				String n = authToken.xid.getName();
				if (n.endsWith("good-loop.com"))
					showEmailAndAddress = true;
				if (n.equals("sanjay@sogive.org"))
					showEmailAndAddress = true;
				if (n.equals("candice.spendelow@gmail.com"))
					showEmailAndAddress = true;
			}
			// SAFETY HACK
			if (!showEmailAndAddress)
				throw new WebEx.E403(
						"Only admins can use this page for now. No admin user listed in auth-tokens: " + tokens);
		...
		// So grab (or scrape a proxy name from email if necessary) before we scrub
		// other PII
		...
		if (!anonymous) {
			...
			if (donorName == null && donor != null)
				donorName = donor.getName();

			// Still no name? Fall back to email addresses, but just take everything up to
			// the @ for privacy
			// This mimics the name-reconstructing behaviour used on the front end
			if (donorName == null) {
				String donorEmail = donation.getDonorEmail();
				if (donorEmail == null && donor != null)
					donorEmail = donor.id;
		...
		if (!showEmailAndAddress) {
			...
			donation.setVia(null); // The fundraiser owner's email also probably counts as PII, even though it's
...
//			donation.setTip(null);
	...
	protected ESQueryBuilder doList4_ESquery_custom(WebRequest state) {
		...
		String q = state.get(SearchServlet.Q); // NB: q is NOT processed in this method - just sanity checked - see
												// super.doList()
	...
	protected JThing doPublish(WebRequest state, KRefresh forceRefreshIgnoredToTrue, boolean deleteDraftIgnoredToTrue)
		...
		if (donation.getAmount().getValue100p() <= 0) {
			throw new WebEx.E400("Donations must be for a positive amount: " + donation.getAmount() + " (donation id: "
					+ donation.getId());
		}
	...
	private void setupRepeat(Donation donation) {
		ESPath path = AppUtils.getPath(null, RepeatDonation.class, RepeatDonation.idForDonation(donation),
				KStatus.PUBLISHED);
