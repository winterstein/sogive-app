	...
	private void updateFundRaiser(Donation donation, String frid, KStatus status) {
		try {
			...
			if (dons.contains(donation.getId())) {
				Log.d(getName(), "skip (already seen) updateFundRaiser " + donation + " frid: " + frid + " status: "
						+ status + " ...");
				return;
			}
			dons.add(donation.getId()); // NB: WARNING: If there is then an exception below, this code will not get
										// re-run
			...
			try {
				...
				if (event != null && event.getMatchedFunding() != 0) {
					...
					// round to the penny
					ma = Math.round(ma * 100) / 100.0; // trailing .0 coerces divisor to a float so we don't get
														// long/int division
					...
					if (cons != null) {
						MoneyItem dupe = Containers.first(cons,
								con -> con != null && "matched funding".equals(con.getText()));
						if (dupe != null) {
							throw new IllegalStateException(
									"Skip Duplicate matched funding in " + donation.getId() + " " + donation);
						}
					}
					// end paranoia
					...
					if (status == KStatus.PUBLISHED) { // this method is called twice for status=draft/published), but
														// we only save Donation the once
			...
			if (donation.getGenerator() == null) {
				...
				Log.d(getName(),
						"updateFundRaiser count: " + fundraiser.getDonationCount() + " total: "
								+ fundraiser.getDonated() + " from " + prevTotal + " for " + fundraiser.getId()
								+ " by donation " + donation.getId());
			}
